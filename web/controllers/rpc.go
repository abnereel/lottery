...
import (
	...
	"github.com/abnereel/lottery/rpc"
...
func (serv *rpcServer) checkParams(uid int64, username string, ip string, now int64, app string, sign string) error {
	...
	str := fmt.Sprint("uid=%d&username=%s&ip=%s&now=%d&app=%s", uid, username, ip, now, app)
...
func (serv *rpcServer) DoLuck(ctx context.Context, uid int64, username string, ip string, now int64, app string, sign string) (r *rpc.DataResult_, err error) {
